...
import { Play, ChevronDown, ChevronRight, Clock, Zap, Database, Activity, Code, Brain, X, StopCircle } from 'lucide-react';
import { useAgents } from '../lib/queries/agents';
import { useRunPlayground, useCancelPlayground } from '../lib/queries/playground.ts';
...
function SpanRow({ span, depth, isExpanded, onToggle }: SpanRowProps) {
  ...
  return (
    <div className="border-b border-gray-800">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-white truncate">{span.name}</span>
      ...
      {/* Expanded details */}
      {isExpanded && (
        <div className="bg-[#0D0E0E] px-4 py-3 border-t border-gray-800" style={{ paddingLeft: `${depth * 24 + 16}px` }}>
          <div className="space-y-3 text-sm">
            {/* For llm_usage - show provider, model, token details */}
            {span.event_type === 'llm_usage' && span.metadata && (
              <div>
                <div className="text-xs font-semibold text-gray-400 mb-1">LLM Usage</div>
            ...
            {span.event_type !== 'llm_usage' && span.event_type !== 'autonomous_think' && 
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div>
                  ...
                  <span className="ml-2 text-gray-300">{new Date(span.start_time).toLocaleTimeString()}</span>
                </div>
              </div>
            </div>
          </div>
...
function SpanTreeNode({ span, depth }: { span: Span; depth: number }) {
  ...
  return <SpanRow span={span} depth={depth} isExpanded={isExpanded} onToggle={() => setIsExpanded(!isExpanded)} />;
}
...
export default function Playground() {
  ...
  const { traces, isConnected, isComplete } = usePlaygroundTraces(activePlaygroundId);
  ...
  return (
    <div className="h-[calc(100vh-56px)] flex flex-col bg-[#191A1A]">
      ...
      <div className="flex-none px-6 py-4 border-b border-gray-800">
        ...
        <p className="text-sm text-gray-400 mt-1">Test your agents with custom prompts</p>
      </div>
...
Example: Research the top 5 programming languages in 2024 and write a comparison report."
          ...
          <div className="flex-none px-6 py-4 border-t border-gray-800 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={handleRun}
                disabled={runPlayground.isPending || !promptText.trim() || (mode === 'sequential' && selectedAgents.length < 2)}
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-3 p-3 bg-[#1F2121] border border-gray-800 rounded-lg cursor-pointer hover:border-[#1FB8CD] transition-colors">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-gray-400">Run with one agent</div>
                </div>
              </label>
              <label className="flex items-center gap-3 p-3 bg-[#1F2121] border border-gray-800 rounded-lg cursor-pointer hover:border-[#1FB8CD] transition-colors">
                <input
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-gray-400">Chain multiple agents</div>
                </div>
              </label>
            </div>
          </div>
            ...
            {mode === 'single' ? (
              ...
              <div className="space-y-2">
                {agents && agents.map((agent) => {
                  ...
                  const order = isSelected ? selectedAgents.indexOf(agent.name) + 1 : null;
                ...
                {selectedAgents.length > 0 && (
                  <div className="mt-3 p-3 bg-[#1F2121] border border-gray-800 rounded-lg">
                    <div className="text-xs text-gray-400 mb-1">Execution order:</div>
                    <div className="text-sm text-white">{selectedAgents.join(' → ')}</div>
                  </div>
                )}
              </div>
            )}
